...
from .config import CNN_TRANSFORMER_MODEL_PATH
from .dataset_service import list_training_entries
from .modeling import CNNTransformerClassifier
...
class CNNTransformerTrainingService(TrainingService):
    def bootstrap_model(self) -> None:
        transformer_model = CNNTransformerClassifier(num_classes=len(self.species)).to(DEVICE)
        try:
            if try_load_checkpoint(transformer_model, CNN_TRANSFORMER_MODEL_PATH):
                self.set_active_model(transformer_model, "CNN + Transformer", True)
                self.append_log("已加载 CNN + Transformer 权重")
                return
        except Exception as error:
            self.append_log(f"检测到旧权重但加载失败，将以未训练模型启动: {error}")

        self.set_active_model(transformer_model, "CNN + Transformer", False)
    ...
    def run_training_job(self, config: TrainConfig) -> None:
        try:
            ...
            model_ref = CNNTransformerClassifier(num_classes=len(self.species)).to(DEVICE)
            ...
            phase_cycle = ["input", "window", "mel", "cnn", "sequence", "transformer", "head"]
            self.append_log(
            ...
            if best_state_dict is not None:
                ...
                self.set_active_model(model_ref, "CNN + Transformer", True)
                self.append_log("训练结束，已切换到最新的 CNN + Transformer 权重")
...
transformer_training_service = CNNTransformerTrainingService()
